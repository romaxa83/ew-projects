<?php

namespace App\Services\Catalog\Service;

use App\DTO\Catalog\Service\DriverAgeDTO;
use App\DTO\Catalog\Service\DriverAgeEditDTO;
use App\DTO\NameTranslationDTO;
use App\Models\Catalogs\Service\DriverAge;
use App\Models\Catalogs\Service\DriverAgeTranslation;
use App\Services\BaseService;
use DB;

class DriverAgeService extends BaseService
{

    public function __construct()
    {}

    public function create(DriverAgeDTO $dto): DriverAge
    {
        DB::beginTransaction();
        try {

            $model = new DriverAge();
            $model->sort = $dto->getSort();
            $model->active = $dto->getActive();

            $model->save();

            foreach ($dto->getTranslations() as $translation){
                /** @var $translation NameTranslationDTO */
                $t = new DriverAgeTranslation();
                $t->driver_age_id = $model->id;
                $t->lang = $translation->getLang();
                $t->name = $translation->getName();
                $t->save();
            }

            DB::commit();

            return $model;
        } catch (\Throwable $e) {
            DB::rollBack();
            \Log::error($e->getMessage());
            throw new \Exception($e->getMessage());
        }
    }

    public function edit(DriverAgeEditDTO $dto, DriverAge $model): DriverAge
    {
        DB::beginTransaction();
        try {

            $model->sort = $dto->getSort() ?? $model->sort;
            $model->active = $dto->getActive() ?? $model->active;
            $model->save();

            $this->editTranslationsName($model, $dto);

            DB::commit();

            return $model;
        } catch (\Throwable $e) {
            DB::rollBack();
            \Log::error($e->getMessage());
            throw new \Exception($e->getMessage());
        }
    }
}
