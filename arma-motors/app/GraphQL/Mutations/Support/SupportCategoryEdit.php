...
class SupportCategoryEdit extends BaseGraphQL
{
    ...
    public function __invoke($_, array $args): Category
    {
        ...
        try {
            ...
            $model = $this->service->edit(
                SupportCategoryEditDTO::byArgs($args),
                $this->repository->findByID($args['id'])
            );
            // @todo dev-telegram
            TelegramDev::info("Админ отредактировал категорию для тех. поддержки ({$model->current->name})", $user->name);
