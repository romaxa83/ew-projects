...
class OrderBodyCreate extends BaseGraphQL
{
    ...
    public function __invoke($_, array $args): Order
    {
        ...
        try {
            ...
            $service = $this->serviceRepository->getBodyService();
            $args['serviceId'] = $service->id;

            $dto = OrderBodyDTO::byArgs($args);
            $order = $this->service->createBody($dto, $user);

            OrderLogger::info('CREATE [body] by data', $args);
            ...
            TelegramDev::info("Пользователь создал заявку на кузовной ремонт", $user->name);
