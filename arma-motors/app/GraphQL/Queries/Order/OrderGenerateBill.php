...
class OrderGenerateBill extends BaseGraphQL
{
...
//            $res = $this->requestService->getDataInvoice($order->uuid);
//            $data = $res->data;

            // test data
            $data = GetInvoice::testData();

            $this->fileService->generateOrderPDF($order, $data, Order::FILE_BILL_TYPE);

            $order->refresh();

            return $this->successResponse(__("message.order.generate bill"));
        } catch (\Throwable $e) {
