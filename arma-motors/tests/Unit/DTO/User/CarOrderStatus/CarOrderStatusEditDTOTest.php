...
namespace Tests\Unit\DTO\User\CarOrderStatus;

use App\DTO\NameTranslationDTO;
use App\DTO\User\CarOrderStatus\CarOrderStatusEditDTO;
use Tests\TestCase;

class CarOrderStatusEditDTOTest extends TestCase
{
    /** @test */
    public function check_fill_by_args()
    {
        $data = [
            ...
            'translations' => [
                'ru' => [
                    'lang' => 'ru',
                    'name' => 'order status edit ru'
                ],
                'uk' => [
                    'lang' => 'uk',
                    'name' => 'order status edit uk'
                ]
            ]
        ];

        $dto = CarOrderStatusEditDTO::byArgs($data);
