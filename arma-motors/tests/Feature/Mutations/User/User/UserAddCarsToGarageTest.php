...
class UserAddCarsToGarageTest extends TestCase
{
    ...
    public function add_to_garage()
    {
        ...
        $this->assertTrue($response->json('data.userAddCarsToGarage.cars.0.inGarage'));
    ...
    public static function getQueryStr(array $data): string
    {
        return sprintf('
            mutation {
                userAddCarsToGarage(input: {carIds: [%d]})
                {
                    id
                    cars {
                        id
