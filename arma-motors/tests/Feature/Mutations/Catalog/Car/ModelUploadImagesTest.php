...
use Tests\Feature\Queries\Catalog\Car\GetOneModelTest;
use Tests\TestCase;
use Tests\Traits\AdminBuilder;
use Tests\Traits\Statuses;

class ModelUploadImagesTest extends TestCase
{
    ...
    public function upload_images()
    {
        ...
        $model = Model::query()->orderBy(\DB::raw('RAND()'))->first();

        $this->assertEmpty($model->image);

        $response = $this->post('graphql',
            [
                'operations' => '{"query": "mutation ($images: [Upload]) {imagesStore(model: '. $this->image_model_model .', modelId: ' . $model->id . ', images: $images) {id, basename, hash, position, mime, url}}"}',
        ...
        $responseLook = $this->graphQL(GetOneModelTest::getQueryStr($model->id));
        $responseLookData = $responseLook->json('data.model');
