...
class TypeController extends BaseStatisticController
{
    /**
     * @SWG\Get(
     *     path="/api/statistic/type/filter",
     *     summary="Получение type для фильтров статистики по size",
     *     tags={"Statistic type"},
     *     security={{"passport": {}}},
     ...
     * )
     */
    public function filterType(RequestReportCount $request)
    {
        try {
            $year = $request['year'];
            ...
            return $this->successJsonMessage($this->typeData($year, $status, $dealer, $eg, $md));
        } catch (\Exception $error){
            return $this->errorJsonMessage($error->getMessage());
        }
    }

    protected function typeData($year, $status, $dealer, $eg, $md): array
    {
        $models = Report::query()
            ->with([
                'user.dealer',
...
//    public function filterType(Statistic\Report\FilterMd $request)
//    {
//        try {
//            $year = $request['year'];
//            $status = $this->requestStatusData($request['status']);
...
//            $eg = $this->requestEgData($request['eg'], $dealer, $status, $year);
//
//            return $this->successJsonMessage($this->typeData($year, $status, $dealer, $eg));
//        } catch (\Exception $error){
...
//    }
//
//    protected function typeData($year, $status, $dealer, $eg): array
//    {
//        $models = Report::query()
//            ->with([
//                'user.dealer',
...
//    public function filterMd(Statistic\Type\FilterMD $request)
//    {
//        try {
//            $year = $request['year'];
//            $status = $this->requestStatusData($request['status']);
...
//            $dealer = $this->requestDeaelrData($request['dealer'], $year, $status, $country);
//            $eg = $this->requestEgData($request['eg'], $dealer, $status, $year);
//            $type = $this->requestTypeData($request['type'], $year, $status, $dealer, $eg);
//
//            return $this->successJsonMessage($this->mdData($year, $status, $dealer, $eg, $type));
//        } catch (\Exception $error){
//            TelegramDev::error(__FILE__, $error);
//            return $this->errorJsonMessage($error->getMessage());
//        }
...
//
    ...
    )
    {
        $reports = Report::query()
            ...
            ->whereHas('reportMachines', function($q) use ($eg){
                $q->whereIn('equipment_group_id', $eg)
//                    ->whereHas('modelDescription.product', function ($q) use($type) {
//                        $q->whereIn('type', $type);
//                    })
                ;
            })
    ...
    /**
     * @SWG\Get(
     *     path="/api/statistic/type/reports",
     ...
     * @throws \Exception
     */
    public function forReports(Statistic\Type\Report $request)
    {
        try {
            $year = $request['year'];
            $status = $this->requestStatusData($request['status']);
            ...
            $eg = $this->requestEgData($request['eg'], $dealer, $status, $year);
            $md = $this->requestMdData($request['md'], $dealer, $eg, $status, $year);
            $type = $this->requestTypeData($request['type'], $year, $status, $dealer, $eg, $md);
            ...
            $reports = Report::query()
                ...
                ->whereHas('reportMachines.modelDescription.product', function($q) use ($type){
                    $q->whereIn('type', $type);
                })
                ->get()
                ->toArray()
    ...
    protected function requestTYPEMdData(string $value, $year, $status, $dealer, $eg, $type): array
    {
        if($value === self::ALL){
            return array_flip($this->mdData($year, $status, $dealer, $eg, $type));
        }
