...
namespace Tests\Feature\Api\Statistics\Size;
...
class FilterSizeTest extends TestCase
{
    ...
    public function success()
    {
        ...
        $this->getJson(route('api.statistic.size.filter', $data))
            ->assertJson($this->structureSuccessResponse([
                $md_1->product->id => $md_1->product->size_name . ' (2)'
            ]))
        ;
    }
    ...
    public function success_some_fields()
    {
        ...
        $data = [
            'year' => Carbon::now()->year,
            'status' => ReportStatus::CREATED.','.ReportStatus::EDITED.','.ReportStatus::VERIFY,
            'country' => $fr.','.$de,
            'dealer' => $dealerFR_1->id.','.$dealerFR_2->id.','.$dealerDE->id,
            'eg' => $md_1->equipmentGroup->id.','.$md_2->equipmentGroup->id.','.$md_2->equipmentGroup->id,
            'md' => $md_1->id.','.$md_1_1->id.','.$md_2->id
        ];

        $this->getJson(route('api.statistic.size.filter', $data))
            ->assertJson($this->structureSuccessResponse([
                $md_1->product->id => $md_1->product->size_name . ' (4)',
                $md_1_1->product->id => $md_1_1->product->size_name . ' (1)',
                $md_2->product->id => $md_2->product->size_name . ' (1)',
            ]))
        ;
    }
