...
class PushNotyToUsersTest extends TestCase
{
    ...
    public function success_template_planed(): void
    {
        ...
        $this->assertFalse($rep_1->pushData->is_send_week);
        $this->assertNull($rep_1->pushData->prev_planned_at);

        $this->artisan('jd:report:push')
            ->assertExitCode(0)
        ;

        $rep_1->refresh();

        $this->assertTrue($rep_1->pushData->is_send_week);
