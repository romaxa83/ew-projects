...
class PushNotyInDayEndTest extends TestCase
{
    ...
    public function success_template_planed(): void
    {
        ...
        $this->assertFalse($rep_1->pushData->is_send_end_day);
        $this->assertNull($rep_1->pushData->prev_planned_at);

        $this->artisan('jd:report:push-in-end-day')
            ->assertExitCode(0)
        ;

        $rep_1->refresh();

        $this->assertTrue($rep_1->pushData->is_send_end_day);
