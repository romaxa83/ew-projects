...
#ключи mysqldump
# если выдает ошибку "Unknown table 'COLUMN_STATISTICS' in information_schema", добавить в OPT - "--column-statistics=0"
OPTS="--add-drop-database --column-statistics=0 --add-locks --skip-tz-utc --create-options --disable-keys --extended-insert --single-transaction --quick --set-charset --routines --events --triggers --comments --quote-names --order-by-primary --hex-blob"
...
if [ "$TYPEHOST" == $test ];then
  # stage
    mysqldump -v ${OPTS} -h 195.201.39.33 -u wezom_jddemodb -pIXoDqhTlbm wezom_jddemodb > ${PATHDUMP}
else
  # prod
    mysqldump -v ${OPTS} -h 52.174.55.1 -u jd_demo_u -pxeeVeiShiej4ih7K jd_demodb > ${PATHDUMP}
fi
ENDIMPORT=$(date +%s)
...
if [ $EXPORT -ne 1 ]; then
  ...
  STARTEXPORT=$(date +%s)
  mysql -v -h 192.168.144.1 -u root -ppassword jd < $PATHDUMP
  ENDEXPORT=$(date +%s)
