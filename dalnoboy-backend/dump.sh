...
if [ "$TYPEHOST" == $test ];then
  # stage
#  mysqldump -v ${OPTS} -h 195.201.39.33 -u arma_db_u -pPRgarMa3zSHHvEJB4nNrSIPa arma_db --ignore-table=arma_db.telescope_entries --ignore-table=arma_db.telescope_entries_tags > ${PATHDUMP}
  mysqldump -v -h 65.21.155.103 -u cms7dev_dalnoboy -pLeiy1EicaiNgiuxooveit4vu cms7dev_dalnoboy --ignore-table=arma_db.telescope_entries --ignore-table=arma_db.telescope_entries_tags > ${PATHDUMP}
else
  # prod
  mysqldump -v ${OPTS} -h ikdb.dalnoboy.com -u dalnoboy_db_u -pxono2zae4Ebahl6aech5Oosi dalnoboy_db > ${PATHDUMP}
fi
ENDIMPORT=$(date +%s)
...
if [ $EXPORT -ne 1 ]; then
  ...
  STARTEXPORT=$(date +%s)
  mysql -v -h 192.168.172.1 -u root -ppassword db < $PATHDUMP
  ENDEXPORT=$(date +%s)
