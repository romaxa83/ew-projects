...
abstract class TestCase extends \Illuminate\Foundation\Testing\TestCase
{
    ...
    protected function loginAsUser(User $user = null): User
    {
        ...
        $this->actingAs($user, GuardsEnum::USER);

        return $user;
    }

    protected function loginAsUserWithRole(User $user = null): User
    {
        $user = $this->loginAsUser($user);
    ...
    protected function loginAsAdmin(Admin $admin = null): Admin
    {
        ...
        $this->actingAs($admin, GuardsEnum::ADMIN);

        return $admin;
    }

    protected function loginAsAdminWithRole(?Admin $admin = null): Admin
    {
        $admin = $this->loginAsAdmin($admin);

        $admin->assignRole(
            Role::whereName(AdminRolesEnum::SUPER_ADMIN)
                ->first()
        );
        return $admin;
    }
