...
class PermissionFilterServiceTest extends TestCase
{
    ...
    public function test_user_get_empty_permissions(): void
    {
        ...
        $permissions = collect(
            [
                Permission::firstOrCreate(['name' => UserCreatePermission::KEY]),
                Permission::firstOrCreate(['name' => UserShowPermission::KEY]),
            ]
        );
        ...
        self::assertTrue(
            $permissions->where('name', UserShowPermission::KEY)
                ->isNotEmpty()
        );
    }
