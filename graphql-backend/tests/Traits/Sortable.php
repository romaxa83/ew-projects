...
trait Sortable
{
    ...
    private function testTranslateSortableFields(array $fields, string $modelClass, callable|null $callback = null): void
    {
        $perPage = 5;
        $directions = ['asc', 'desc'];
        ...
        foreach ($fields as $field) {
            foreach ($directions as $direction) {
                $query = sprintf(
                    'query { %s ( per_page: %s sort: "%s" ) { data {translate { ' . $field . '} } } }',
                    self::QUERY,
                    $perPage,
                    "$field-$direction"
                );
                ...
                /** @var Model|ModelMain $model */
                $model = app($modelClass);
                $modelTable = $model->getTable();
                $translateTable = $model->getTranslateTableName();
                ...
                $sorted = $modelClass::query()
                    ...
                    ->where('language', default_language()->slug)
