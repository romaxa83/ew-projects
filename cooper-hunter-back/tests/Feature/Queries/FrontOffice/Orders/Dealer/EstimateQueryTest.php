...
class EstimateQueryTest extends TestCase
{
    use DatabaseTransactions;

    public const MUTATION = DealerProduct\EstimateQuery::NAME;
        ...
        $this->postGraphQL([
            ...
            ->assertJson([
                'data' => [
                    self::MUTATION => [
                        'type' => 'success',
                        'message' => env('APP_URL') . "/storage/dealer-order-pdf/Order-estimate-{$order->id}.pdf"
                    ]
                ]
            ])
        ;

        unlink($order->getEstimateStoragePath());
    }

    /** @test */
    public function get_invoice_url(): void
    {
        $now = Carbon::now();
        CarbonImmutable::setTestNow($now);
        ...
        $this->postGraphQL([
            ...
            ->assertJson([
                'data' => [
                    self::MUTATION => [
                        'type' => 'success',
                        'message' => env('APP_URL') . "/storage/dealer-order-pdf/Order-invoice-{$order->id}.pdf"
                    ]
                ]
            ])
        ;

        unlink($order->getInvoiceStoragePath());
    }

    /** @test */
    public function fail_generate_invoice_not_has_invoice(): void
    {
        $now = Carbon::now();
        CarbonImmutable::setTestNow($now);
        ...
        $this->postGraphQL([
            ...
            ->assertJson([
                'data' => [
                    self::MUTATION => [
                        'type' => 'warning',
                        'message' => __('exceptions.dealer.order.not has invoice', ['guid' => $order->guid])
                    ]
                ]
            ])
        ;
