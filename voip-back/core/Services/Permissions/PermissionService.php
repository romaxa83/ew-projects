...
use Core\Repositories\Permissions\RoleRepository;
...
class PermissionService
{
    ...
    public function deleteRole(Role $role): bool
    {
        if($role->isSuperAdmin()){
            throw new RoleException(__("exceptions.roles.cant_delete_role"));
        }

        /** @var $adminRepo AdminRepository */
        $adminRepo = resolve(AdminRepository::class);
        if($adminRepo->hasAdminWithRole($role)){
            throw new RoleException(__("exceptions.roles.cant_delete_role_attach_user"));
        }
    ...
    public function firstOrCreateEmployee(): Role
    {
        $name = config('permission.roles.employee');
        $guard = Employee::GUARD;

        return $this->roleRepository->getByNameAndGuard($name, $guard)
            ?: $this->createDefaultRole($name, $guard);
    }

    public function createDefaultRole(string $name, string $guard): Role
    {
        $role = Role::query()->firstOrCreate([
            'name' => $name,
            'guard_name' => $guard,
