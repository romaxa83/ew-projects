...
class DeviceBatteryStopChargingAlertTest extends TestCase
{
    ...
    public function test_battery_stop_charging_alert(): void
    {
        ...
        $this->createLogWithChargingBattery($truck->gpsDevice->imei);
        $this->service->processMessages();
        $this->assertEquals(0, $this->getStopChargingBatteryAlertsCount($truck->id));

        $this->createLogWithChargingBattery($truck->gpsDevice->imei);
        $this->service->processMessages();
        $this->assertEquals(0, $this->getStopChargingBatteryAlertsCount($truck->id));

        $this->createLogWithStopChargingBattery($truck->gpsDevice->imei);
        $this->service->processMessages();
        $this->assertEquals(1, $this->getStopChargingBatteryAlertsCount($truck->id));
        ...
        $this->createLogWithStopChargingBattery($truck->gpsDevice->imei);
        $this->service->processMessages();
        $this->assertEquals(2, $this->getStopChargingBatteryAlertsCount($truck->id));

    }

    protected function createLogWithStopChargingBattery(string $imei): Message
    {
    ...
    protected function createLogWithChargingBattery(string $imei): Message
    {
        return Message::factory()->create([
            'imei' => $imei,
            'received_at' => now(),
    ...
    private function getStopChargingBatteryAlertsCount(int $truckId): int
    {
        return Alert::query()
            ->where('truck_id', $truckId)
            ->where('alert_type', Alert::ALERT_TYPE_DEVICE_BATTERY)
            ->where('alert_subtype', Alert::ALERT_SUBTYPE_BATTERY_STOP_CHARGING)
            ->count();
    }
