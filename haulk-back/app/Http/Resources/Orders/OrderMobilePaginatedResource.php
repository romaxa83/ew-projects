 ...
 * @mixin Order
 */
class OrderMobilePaginatedResource extends JsonResource
{
    /**
     * @OA\Schema(schema="OrderMobilePaginatedResource",
        ...
        return array_merge(
            [
                ...
                'pickup_customer_signature' => FileResource::make(
                    $this->getFirstMedia(Order::PICKUP_CUSTOMER_SIGNATURE_COLLECTION_NAME)
                ),
                'pickup_driver_signature' => FileResource::make(
                    $this->getFirstMedia(Order::PICKUP_DRIVER_SIGNATURE_COLLECTION_NAME)
                ),
                ...
                'delivery_customer_signature' => FileResource::make(
                    $this->getFirstMedia(Order::DELIVERY_CUSTOMER_SIGNATURE_COLLECTION_NAME)
                ),
                'delivery_driver_signature' => FileResource::make(
                    $this->getFirstMedia(Order::DELIVERY_DRIVER_SIGNATURE_COLLECTION_NAME)
                ),
                Order::ATTACHMENT_COLLECTION_NAME => $this->getMedia(Order::ATTACHMENT_COLLECTION_NAME)
                    ? FileResource::collection($this->getMedia(Order::ATTACHMENT_COLLECTION_NAME)->all())
                    : null,
                'documents' => $this->getMedia(Order::DRIVER_DOCUMENTS_COLLECTION_NAME)
                    ? FileResource::collection($this->getMedia(Order::DRIVER_DOCUMENTS_COLLECTION_NAME)->all())
                    : null,
                'photos' => ImageResource::collection($this->getMedia(Order::DRIVER_PHOTOS_COLLECTION_NAME)),
                'driver_comments' => OrderCommentListResource::collection($this->comments),
