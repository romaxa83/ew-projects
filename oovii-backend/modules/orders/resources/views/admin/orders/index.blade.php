...
@section('content')
    <div class="table-responsive">
        <table class="table table-striped table-hover">
            <thead>
            <tr>
                <th width="1%">@massControl(compact('routeName', 'deleteText'))</th>
                <th width="1%">#</th>
                ...
                <th>@lang('cms-orders::admin.orders.Provider')</th>
                <th>@lang('cms-orders::admin.orders.Status')</th>
                <th>@lang('cms-orders::admin.orders.Date')</th>
                <th>@lang('cms-orders::admin.orders.Payed')</th>
            ...
            @foreach($result as $obj)
                <tr>
                    <td>@massCheck($obj)</td>
                    <td>
                        @editResource(['obj'=> $obj, 'text' => $obj->id])
                    </td>
                    <td>
                        @if($obj->client)
                            @if($obj->user)
                                @editResource(['obj'=> $obj->user, 'text' => $obj->client->full_name, 'ability' => 'users.edit', 'route' => 'admin.users.edit', 'target' => '_blank'])
                            @else
                    ...
                    <td>{{ $obj->quantity . ' / ' . money($obj->whole_purchase_price, true) }}</td>
                    <td>
                        {{ $obj->provider->name }} / {{ $obj->provider->company }}
                    </td>
                    <td>
                    ...
                    <td>
                        {{ $obj->payed ? __('cms-core::admin.layout.Yes') : __('cms-core::admin.layout.No') }}
                    </td>
                    <td>
