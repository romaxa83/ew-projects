...
class CollectionResource extends JsonResource
{
    public function toArray($request): array
    {
        ...
        return [
            ...
            'startAt' => $model->start_at ? $model->start_at->format(config('cms.core.time.format.start_at.api')) : null,
            'isStartCounter' => $model->start_counter,
            'endAt' => $model->end_at ? $model->end_at->format(config('cms.core.time.format.end_at.api')) : null,
            'isEndCounter' => $model->end_counter,
            'createdAt' => $model->created_at->format(config('cms.core.time.format.created_at.api')),
            'type' => $model->type,
            'products' => ProductSimpleResource::collection($model->availableProducts)
                ->setCollectionData(Product::formatCollectionDataForProduct($model->resource)),
     ...
     * @OA\Property(property="products", title="Products", description="Товары", type="array",
