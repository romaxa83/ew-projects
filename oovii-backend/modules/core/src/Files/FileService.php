...
class FileService
{
     ...
     * @return mixed
     */
    public function getFileLastModified(Model $model, string $field = self::FILE, string $locale = null): ?Carbon
    {
        if (!$this->fileExists($model, $field, $locale)) {
            return null;
        }
        ...
        $timestamp = $this->getStorage($setting)
            ->lastModified($this->buildPath([array_get($setting, 'directory'), $model->getAttribute($field)]));
