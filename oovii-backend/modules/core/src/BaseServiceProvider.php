...
class BaseServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        if (false === $this->app->routesAreCached()) {
        ...
        if ($this->app->bound('livewire') && !$this->app[LivewireComponentsFinder::class]->componentsIsCached()) {
    ...
    {
        $files = glob($this->root('routes/*.php'));
        if ($files === false) {
            return;
        }
    ...
    {
        $moduleName = $this->moduleName();
        $prefix = str_replace('-', '/', $moduleName);

        // Configs
        if (is_dir($this->root('config'))) {
            $configs = [];

            $files = glob($this->root('config/*.php'));
            if ($files !== false) {
                foreach ($files as $file) {
    ...
    protected function registerListeners()
    {
        ...
        Event::listen('livewire:discover', [$this, 'registerLivewireComponents']);

        if ($this->app->runningInConsole()) {
            if ($this->methodExist('sitemapXml')) {
    ...
    {
        $livewireDir = $this->root('src/Http/Livewire');
        if (!is_dir($livewireDir)) {
            return;
        }

        collect(File::allFiles($livewireDir))
            ->map(function (SplFileInfo $file) use ($livewireDir) {
                return Str::of($file->getPathname())
                    ->after($livewireDir)
                    ->replace(['/', '.php'], ['\\', ''])
                    ->prepend("{$this->namespace()}\\Http\\Livewire");
            })
            ...
            ->each(function (string $class) {
                $alias = Str::of($class)
                    ->after("{$this->namespace()}\\Http\\Livewire")
                    ->explode('\\')
                    ->filter()
                    ->map(function ($part) {
                ...
                $this->app['livewire']->component(str_replace('cms-', '', $this->moduleName()) . '.' . $alias, $class);
            });
    }
}
