...
trait PublishedTrait
{
    ...
    public function scopePublished($query)
    {
        ...
        if (method_exists($this, 'translate') && $this->isTranslationAttribute($field)) {
            ...
            $query->where($this->getTable() . '.' . $field, true);
        }
    ...
    public function scopePublishedWithSlug($query, $slug, $slugField = 'slug')
    {
        ...
        if (method_exists($this, 'translate')) {
            ...
            if (!empty($multilingualFields)) {
                $query->whereHas('translation', function ($subQuery) use ($multilingualFields) {
                    /** @var Builder $subQuery */
                    $subQuery->where($multilingualFields);
                });
            }
            if (!empty($baseFields)) {
            ...
            $query->where($this->getTable() . '.' . $field, true)->where($slugField, $slug);
        }
