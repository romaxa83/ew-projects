...
trait SettingControllerTrait
{
     ...
     * @throws \Illuminate\Auth\Access\AuthorizationException
     */
    public function deleteSettingsFile($id, Request $request)
    {
        $this->authorizeForAction('edit-settings');
        ...
        $row->deleteFile($request->input('locale'));
    ...
    protected function initializeStorageFieldWithDefaultValue(AbstractField $field, array $columns)
    {
        ...
        $row->image_settings = method_exists($field, 'extractSettings') ? $field->extractSettings() : [];
