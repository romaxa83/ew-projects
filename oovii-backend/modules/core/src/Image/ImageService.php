...
class ImageService
{
    ...
    protected const WEBP_SUPPORT_KEY = 'webp_supported';
    ...
    public function deleteAllImages(Model $model, bool $save = true): bool
    {
        ...
        foreach (array_keys($model->imageSettings()) as $field) {
            $this->deleteImage($model, $field, self::extractSetting($model, $field), null, $save);
        }
    ...
    {
        $storedFileName = $this->getStoredFileName($model, self::extractSetting($model, $field), $field, $locale);
    ...
    {
        $setting = self::extractSetting($model, $field);
     ...
     * @param  array  $fields
     */
    public function replicate(Model $model, array $fields)
    {
        if (empty($fields)) {
            $fields = array_keys($model->imageSettings());
        }

        $fields = collect($fields)
            ->mapWithKeys(function ($field) use ($model) {
                return [$field => self::extractSetting($model, $field)];
            })
            ->filter(function ($setting) {
                return array_get($setting, 'replicate', true);
            });

        foreach ($fields as $field => $setting) {
            $directory = array_get($setting, 'directory');
            $sizes = array_keys(array_get($setting, 'sizes', []));
            ...
            if ($this->isMultilingual($setting)) {
                foreach ($this->locales as $localLocale) {
                    $newFileName = $this->copyAllImages(
                        $this->getStoredFileName($model, $setting, $field, $localLocale),
                        $originalStorage,
                        $storage,
                        $directory,
                        $sizes
                    ...
                    $model->translateOrNew($localLocale)->setAttribute($field, $newFileName);
                }
            } else {
                $newFileName = $this->copyAllImages(
                    $this->getStoredFileName($model, $setting, $field),
                    $originalStorage,
                    $storage,
                    $directory,
                    $sizes
                ...
                $model->setAttribute($field, $newFileName);
            }
        }
    }
    ...
    public function imageExists(Model $model, string $size = null, string $field = self::IMAGE, string $locale = null)
    {
        ...
        $path = $this->buildPath([array_get($setting, 'directory'), $size, $storedFileName]);

        return $this->getStorageBySize($setting, $size)->exists($path);
    }
    ...
    public function imageSize(Model $model, string $size = null, string $field = self::IMAGE, string $locale = null)
    {
        ...
        $storage = $this->getStorageBySize($setting, $size);
    ...
    {
        $sizes = array_get(self::extractSetting($model, $field), 'sizes', []);
     ...
     * @param  string|null  $locale
     * @return string
     * @throws IncorrectImageSizeException|Throwable
     */
    public function getImageUrl(Model $model, string $size = null, string $field = self::IMAGE, string $locale = null)
    {
        ...
        if (static::ORIGINAL === $size) {
            return $this->getOriginalStorage($setting)->url($path);
        }
        ...
        $pathWebp = $path . '.webp';
        if (static::webPSupport() && $storage->exists($pathWebp)) {
    ...
    {
        $settings = self::extractSetting($model, $field);
    ...
    public static function webPSupport(): bool
    {
        ...
        $webpSupported = in_array('image/webp', Request::getAcceptableContentTypes());

        if (Request::hasSession() && ($session = Request::session())->isStarted()) {
            if ($session->has(static::WEBP_SUPPORT_KEY)) {
                return (bool)$session->get(static::WEBP_SUPPORT_KEY);
            }

            if ($webpSupported) {
                $session->put(static::WEBP_SUPPORT_KEY, true);
            }
        }
     ...
     * @return array
     */
    public static function extractSetting(Model $model, string $field = self::IMAGE): array
    {
    ...
    /**
     * @param  SymfonyUploadedFile|string|mixed  $file
     * @param  string|null  $directory
     * @param  FilesystemAdapter  $storage
     * @param  FilesystemAdapter  $originalStorage
     * @param $sizes
    ...
    protected function copyAllImages(
        ...
        $this->copyFileIfExists(
            ...
            "{$directory}/original/{$newFileName}"
        );
     ...
     * @param  string  $target
     */
    protected function copyFileIfExists(FilesystemAdapter $storage, string $source, string $target)
    {
        ...
        // Try copy .webp file.
        if ($storage->exists("{$source}.webp")) {
            $storage->copy("{$source}.webp", "{$target}.webp");
        }
    }
     ...
     * @return FilesystemAdapter
     */
    protected function getStorageBySize(array $setting, ?string $size): FilesystemAdapter
    {
        return $size === static::ORIGINAL ? $this->getOriginalStorage($setting) : $this->getStorage($setting);
    }
}
