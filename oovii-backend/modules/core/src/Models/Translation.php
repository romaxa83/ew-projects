...
class Translation extends Model
{
     ...
     * @var array[]
     */
    protected static $localesBySide;
     ...
     * @return array
     */
    public static function prepareKeys(string $key): array
    {
        $data = [];

        $parts = static::parseKey($key);
        foreach (static::localesBySide($parts['side']) as $locale) {
            $partsLocaled = array_merge($parts, ['locale' => $locale]);
            $data[] = array_merge($partsLocaled, ['text' => static::getDefaultText($locale, $partsLocaled, $key)]);
        }
     ...
     * @return array
     */
    public static function localesBySide(string $side): array
    {
        if (null === static::$localesBySide) {
            static::$localesBySide = [
                TranslationSide::ADMIN => array_keys(config('cms.core.translations.admin.locales', [])),
                TranslationSide::SITE => array_keys(app('locales')),
            ];
        }

        return array_get(static::$localesBySide, $side, []);
    }
}
