...
@switch($row->getType())
    ...
    @case(\WezomCms\Core\Settings\Fields\AbstractField::TYPE_STATUS)
    <div>
    ...
    {!! Form::imageUploader($name, $row->getValueObj(), route($routeName . '.delete-settings-file', [$row->getStorageId(), 'locale' => $locale ?? null]), $row->getAttributes()) !!}
    @break

    @case(\WezomCms\Core\Settings\Fields\AbstractField::TYPE_FILE)
    {!! Form::settingsFileUploader($name, $row->getValueObj(), route($routeName . '.delete-settings-file', [$row->getStorageId(), 'locale' => $locale ?? null]), $row->getAttributes()) !!}
    @break
