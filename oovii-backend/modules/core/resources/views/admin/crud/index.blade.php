    ...
    @if(isset($result) && $result instanceof \Illuminate\Pagination\AbstractPaginator && $result->isNotEmpty())
        <div class="mt-2 d-flex justify-content-md-center">
            <div class="mw-100">
                ...
                <div class="text-center">@lang('cms-core::admin.layout.Total') <strong>{{ number_format($result->total(), 0, '.', ' ') }}</strong> {{ trans_choice('cms-core::admin.layout.record|records', $result->total()) }}</div>
            </div>
        </div>
    @endif
@endsection
