@php
/**
 * @var $obj \WezomCms\Core\Models\Role
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    {!! Form::text(
                        ...
                        null,
                        [
                            'placeholder' => __('cms-core::admin.roles.Name'),
                            'readonly' => $obj->isSystemRole(),
                        ]) !!}
                </div>
            </div>
        </div>
            ...
            <div class="card-body">
                <div class="check-wrapper">
                    <div class="custom-control custom-checkbox">
                        <input type="checkbox" class="js-check-child custom-control-input" id="check-all" {{ $disabled ? 'disabled' : null }}>
                    ...
                    @foreach($permissions as $key => $group)
                        <div>
                            @if(($group['checkboxes'] ?? false) && count($group['checkboxes']) > 1)
                                <div class="check-wrapper">
                                    <div class="custom-control custom-checkbox">
                                        <input type="checkbox" class="js-check-child custom-control-input"
                                               value="" id="check-perm-{{ $key }}"
                                               {{ $disabled ? 'disabled' : null }}>
                                        <label class="custom-control-label" for="check-perm-{{ $key }}">
                                            {{ $group['name'] }}
                                        </label>
                                    </div>
                                        ...
                                        @foreach($group['checkboxes'] as $permission => $name)
                                            <div class="custom-control custom-checkbox mb-1">
                                                <input type="checkbox" name="permissions[]"
                                                       class="custom-control-input" id="permission-{{ $permission }}"
                                                        {{ in_array($permission, $selectedPermissions, true) ? 'checked' : '' }}
                                                        value="{{ $permission }}" {{ $disabled ? 'disabled' : null }}>
                                                <label class="custom-control-label" for="permission-{{ $permission }}">
                                                    {{ $name }}
                                                </label>
                                            </div>
                                        @endforeach
                            ...
                            @else
                                ...
                                <div class="custom-control custom-checkbox mb-1">
                                    <input type="checkbox" name="permissions[]"
                                           ...
                                           value="{{ $key }}" {{ $disabled ? 'disabled' : null }}>
