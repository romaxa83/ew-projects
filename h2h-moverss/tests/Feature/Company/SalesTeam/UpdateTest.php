...
class UpdateTest extends TestCase
{
    ...
    /** @test */
    public function success_update_sales_plan()
    {
        $this->loginUser();
        ...
        $sales_plan_3_1 = $this->salesPlanBuilder
            ...
            ->employee($employee_3)
            ->create();
        $sales_plan_3_2 = $this->salesPlanBuilder
            ->date($now->subMonths(2))
            ->employee($employee_3)
            ->create();
        ...
        $data = [
            'sales_plans' => [
                ...
                'long' => [
                    [
                        'sales_plan_id' => $sales_plan_3_2->id,
                        'local' => 4444,
                        'intrestate' => $sales_plan_3_2->intrestate,
                    ],
                ]
            ]
        ];

        $this->assertNotEquals($data['sales_plans']['local'][0]['local'], $sales_plan_1_2->local);
        $this->assertNotEquals($data['sales_plans']['local'][0]['intrestate'], $sales_plan_1_2->intrestate);
        $this->assertNotEquals($data['sales_plans']['long'][0]['local'], $sales_plan_3_2->local);

        $this->post(route('company.sales-team.update'), $data)
//            ->dump()
            ->assertJson([
                'success' => true,
                'data' => [
                    'sales_plans' => [
                        'local' => [
                            [
                                ...
                                'sales_plan_id' => $sales_plan_1_2->id,
                                'local' => $data['sales_plans']['local'][0]['local'],
                                'intrestate' => $data['sales_plans']['local'][0]['intrestate'],
                        ...
                        'long' => [
                            [
                                ...
                                'sales_plan_id' => $sales_plan_3_2->id,
                                'local' => $data['sales_plans']['long'][0]['local'],
                                'intrestate' => $sales_plan_3_2->intrestate,
                                'date' => $sales_plan_3_2->getDate(),
    ...
    /** @test */
    public function success_update_sales_plan_nothing()
    {
        $this->loginUser();
    ...
    /** @test */
    public function update_efficiency_plan()
    {
        $this->loginUser();
        ...
        $data = [
            'efficiency_plan' => [
                ...
                'conversion_local_team' => 33,
                'conversion_long_team' => 1,
            ]
        ];

        $this->assertNotEquals(
            $data['efficiency_plan']['conversion_local_team'],
            $efficiencyPlan->conversion_local_team
        );
        $this->assertNotEquals(
            $data['efficiency_plan']['conversion_long_team'],
            $efficiencyPlan->conversion_long_team
        );
            ...
            ->assertJson([
                ...
                'data' => [
                    'efficiency_plan' => [
                        ...
                        'conversion_local_team' => $data['efficiency_plan']['conversion_local_team'],
                        'conversion_long_team' => $data['efficiency_plan']['conversion_long_team'],
