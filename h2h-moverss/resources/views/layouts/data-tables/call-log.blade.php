...
@push('extendFooter')
    ...
    <script>
        ...
        function getCall(id) {
            const el = $(`.details-${id}`),
                callId = el.data('call-id'),
                loaded = el.data('loaded'),
                pbxCallId = el.data('pbx-call-id');
            ...
            axios.get('/orders/recordZadarma?callID=' + callId + '&pbx_callID=' + pbxCallId)
                .then(({data}) => {
                    ...
                    data.links.forEach(function (v) {
                        txt += `<audio controls src="${v}">
                            Your browser does not support the <code>audio</code> element. o_0
                        </audio>`;
                    });

                    el.after('<div>' + txt + '</div>');
                })
                .catch(function (error) {
                    console.log('error', error);
                    App.Forms.showAlert('error', error);
                });
        }
    </script>
@endpush
                ...
                <div class="panel-hdr">
                    ...
                    <div class="panel-toolbar">
                        ...
                        <select class="ml-2 custom-select custom-select-sm" id="user-filter" style="width: 250px;">
                            ...
                            @foreach($users as $v)
                                <option value="{{ $v->id }}">{{ $v->name }} ({{$v->employee->name}} {{$v->employee->l_name}})</option>
                            @endforeach
                        </select>
                    </div>
                </div>
